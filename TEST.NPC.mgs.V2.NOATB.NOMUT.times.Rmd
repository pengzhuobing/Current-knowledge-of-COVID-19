---
title: "Compare significance" #比较单个菌的相关性
output:
  html_notebook:
    code_fold: hide
---
...
# tag
tag <- "NPC.ATBF.times" #分析没有用抗生素的NPC病人
```
...
FDNA.NPC.pick2.ATBF <- FDNA.NPC.pick2%>%filter(ATB02==F)  #挑选出0-2个月没有用抗生素的病人
FDNA.NPC.pick3.ATBF <- FDNA.NPC.pick3%>%filter(ATB02==F)  #FDNA.NPC.pick2,3的区别是2是ws，3是ms
sp.prf <- read.table("../../Result/01.Profile/MGS_V2.profile") #sp.prf是所有样本的mgs的profile
sp.min.abun <- sort(unique(unlist(sp.prf)))[2]  #获得sp.prf中最小的丰度数值
mgs.V2.anno <- read.table("../../Result/01.Profile/00.IGC.MGS.annotation",sep="\t",header=T) #mgs物种注释信息
mgs.V2.anno$mgs.V2 <- sub(":",".",mgs.V2.anno$X)
mgs.V2.anno2 <- mgs.V2.anno[,c(8,2,3)] #mgs物种注释信息，其中包括mgs、物种、Pct
mgs.V2.LPS.anno <- read.csv("../../Result/01.Profile/MGS.1507.annoatation.LPS.csv")
mgs.V2.LPS.anno$MGS <- sub(":",".",mgs.V2.LPS.anno$MGS)
rownames(mgs.V2.LPS.anno) <- mgs.V2.LPS.anno$MGS #mgs和界门纲目科属种的样本信息
```

# Curation
- ../TUMORMETA/Assay/09.ReportScoree subjects with antibiotics usage during baseline (-2month before treatment) to 2nd month after treatment.

**test randomly pick of baseline**
```{r}
all.base.phe <- FDNA.NS%>%filter(Ms=="M0"&ATB02==F)  #FDNA.NS是DNAID的全部信息，包括但不仅限于采样时间、来源，all.base.phe是baseline采样且0-2个月没有用抗生素的样本信息

#randPick随机输出矩阵中PID和DNAID(一列)
randPick <- function(d){ 
...
#     set.seed(i)
#     rand.pick.base.phe[[i]] <- ddply(all.base.phe,"PID",randPick)  #可重复的随机挑选100个M0样本，包括PID和DNAID信息
#   }
#   
#   ####设置R多线程计算####
#   cl <- makeCluster(5) #设置参与并行5个CPU核数目
#   clusterExport(cl,c("aovFun","all.base.phe","rand.pick.base.phe","sp.prf","mgs.V2.anno2")) #添加并行计算中用到的环境变量
#   rand.pick.base.test <- parSapply(cl,1:set.rand.time,FUN=function(x){  #parSapply是apply的并行计算版本
#     library(dplyr);library(plyr);
#     anoFun(sp.prf[,as.character(rand.pick.base.phe[[x]]$DNAID)], #rand.pick.base.test是对随机取得的100个样本mgs丰度进行annova检验的值
...
# 
# 
# adj.pval.stat <- sapply(rand.pick.base.test, "[[", "adj.pval")   ##???[[是啥意思
# adj.pval.stat.df <- data.frame(
#   mgs.V2=rand.pick.base.test[[1]]$mgs.V2,  #pick第一个样品的mgs
...
BE2.p.val.stat.df <- data.frame( #BE2.p.val.stat.df指mgs、平均值、富集方向、显著性等
  mgs.V2=rand.pick.base.BE2[[1]]$mgs.V2,
  mean.pval=rowMeans(BE2.p.val.stat), #算的每行的平均值
...
sign.BE2.mgs <- (BE2.p.val.stat.df%>%filter(sign.freq==100&mean.pval<0.05))$mgs.V2 #挑出显著的mgs
...
```{r}
#WS
if(file.exists("../../Result/TEST/timeSeries/sum.Ws.BE3.kw.test.RData")){
  load("../../Result/TEST/timeSeries/sum.Ws.BE3.kw.test.RData")
}else{
  FDNA.NPC.pick2.ATBF$Ws <- droplevels(FDNA.NPC.pick2.ATBF$Ws) #删除level w18
  sum.Ws.BE3.test <- NULL  #分为3个进程
  for(i in levels(FDNA.NPC.pick2.ATBF$Ws)){
    tsi.phe <- FDNA.NPC.pick2.ATBF%>%filter(Ws==i)
    tsi.phe <- tsi.phe[order(interaction(tsi.phe$Ws,tsi.phe$BE3)),]  #似乎只是排了个序
    rownames(tsi.phe) <- tsi.phe$DNAID 
    tsi.prf <- sp.prf[,as.character(tsi.phe$DNAID)] #tsi.phe的mgs的丰度信息
    ### tsi.BE3.in0.w <- aovFun(tsi.prf,tsi.phe,"DNAID","BE3","BMI",ex.0=F)
    tsi.BE3.in0.w <- tkFun(tsi.prf,tsi.phe,"DNAID","BE3",NULL,ex.0=F) #三个疾病进程需要用到kw检验,这里不要考虑BMI
  ...
  save(sum.Ws.BE3.test,file="../../Result/TEST/timeSeries/NPC.sum.Ws.BE3.kw.test.RData")
}
...
if(file.exists("../../Result/TEST/timeSeries/sum.Ms.BE3.anova.test.RData")){
  ...
  for(i in levels(FDNA.NPC.pick3.ATBF$Ms)){
    ...
    ##tsi.BE3.in0.w <- aovFun(tsi.prf,tsi.phe,"DNAID","BE3","BMI",ex.0=F)
    tsi.BE3.in0.m <- tkFun(tsi.prf,tsi.phe,"DNAID","BE3",NULL,ex.0=F)
    sum.Ms.BE3.test[[i]] <- tsi.BE3.in0.m
    ggplot(tsi.BE3.in0.m,aes(x=FDR,fill=Enrich)) + geom_histogram(position="stack") + xlim(c(0,1))
    # ggsave(paste0("../../Result/TEST/timeSeries/Ms.",i,".FDR.hist.pdf"))
    # ggplot(tsi.BE3.in0.m,aes(x=ori.pval,fill=Enrich)) + geom_histogram(position="stack") + xlim(c(0,1))
    # ggsave(paste0("../../Result/TEST/timeSeries/Ms.",i,".ori.pval.hist.pdf"))
    # ggplot(tsi.BE3.in0.m,aes(x=adj.pval,fill=Enrich)) + geom_histogram(position="stack") + xlim(c(0,1))
    # ggsave(paste0("../../Result/TEST/timeSeries/Ms.",i,".adj.pval.hist.pdf"))
  }
  save(sum.Ms.BE3.test,file="../../Result/TEST/timeSeries/sum.Ms.BE3.kw.test.RData")
}
```
...
i="W0"
tsi.phe <- FDNA.NPC.pick2.ATBF%>%filter(Ws==i) #w0样本信息
tsi.phe <- tsi.phe[order(interaction(tsi.phe$Ws,tsi.phe$BE3)),]
rownames(tsi.phe) <- tsi.phe$DNAID
tsi.prf <- sp.prf[,as.character(tsi.phe$DNAID)] #w0样本profile
dati <- data.frame(t(tsi.prf),tsi.phe)
res.aov <- aov(MGS.igc1507~BE3,data=dati) #做多因素方差分析,为什么挑MGS.igc1507 ?
summary(aov(MGS.igc1507~BE3,data=dati))

sum.raw.aov <- NULL
sum.adj.aov <- NULL
for(i in 1:nrow(tsi.prf)){   #对每一个mgs都做annova方差分析
  tmp <- dati
  colnames(tmp)[i] <- "abun"
  ...
  res <- summary(aov(lgab~BE3,data=tmp))
  newdf <- as.data.frame(res[[1]][2,]) #获得residuals误差
  #newdf <- as.data.frame(res[[1]][1,])
  rownames(newdf) <- colnames(dati)[i]
...
sum.aov$mostGRP <- ifelse(sum.aov$SD.PD<sum.aov$PR.PD,ifelse(sum.aov$SD.PD<sum.aov$PR.SD,"SD.PD","PR.SD"),"PR.PD")
#mostGRP列选取SD.PD/PD.PR/SD.PR中最小的那个
...
select.mgs <- (sum.Ms.BE3.test$M0%>%filter(p.value<0.05&Occ.>0.2))$mgs.V2
...
ggplot(sum.Ms.wilcox.df%>%filter(Pct>20&Occ.>0.2),
       aes(x=p.value,color=Ms)) + geom_density()
```
...
if(do.write){
  ggsave(phMdv,width=8,height=8,
       filename="../../Result/TEST/timeSeries/NPC.mgs.kw.Ms.pheatmap.pdf")
}
```
...
phWd
if(do.write){
  ggsave(phWd,width=8,height=8,
       filename="../../Result/TEST/timeSeries/NPC.mgs.kw.Ws.pheatmap.pdf")
}
```


**Get significant BE3 differential taxonomic annotation & order**
```{r}
...
phMd.r <- pheatmap(sum.Ws.median.matrix[phm.row.order,],cluster_rows = T,cluster_cols = F,
         color=c("#CCCCCC",colorRampPalette(brewer.pal(n = 11, name ="RdYlGn"))(11)),
         gaps_col = c(9,18),display_numbers = sum.Ws.signif.matrix,
         #breaks=rev(c(1,0.7,0.5,0.2,0.1,0.05,0.01,0.005,0.001,0.0005,0))
)
...
#visual
mgs.pct20 <- mgs.V2.anno2$mgs.V2[which(mgs.V2.anno2$Pct>20&mgs.V2.anno2$mgs.V2%in%phMd.row.order)]
#pick.rand.100.mgs.pct20 <- intersect(pick.rand.100.mgs,mgs.pct20)
phMd.r2 <- pheatmap(sum.Ms.median.matrix[mgs.pct20,],
         cluster_rows = T,cluster_cols = F,
         color=c("#CCCCCC",colorRampPalette(brewer.pal(n = 11, name ="RdYlGn"))(11)),
         gaps_col = c(6,12),
         display_numbers = sum.Ms.signif.matrix[mgs.pct20,],
...
if(do.write){
  write.csv(phMd.r.sum.sign.anno,file="../../Result/TEST/timeSeries/NPC.mgs.sum.phmd.gin.anno.csv")
  write.csv(phMd.r.sum.sign.anno[phMd.r2.order,],
            file="../../Result/TEST/timeSeries/NPC.mgs.sum.phmd.pct20.anno.csv")
}
```
...
psPD<- ggplot(select.df%>%filter(Enrich=="PD"),aes(x=Ms,y=Abundance,fill=BE3)) + geom_boxplot() +
  ...
  scale_y_log10() + facet_wrap(MGS~MainTax,ncol=2,scale="free") + scale_fill_brewer(palette = "Set1")
psPD
...
if(do.write){
  ggsave(psa,file="../../Result/TEST/timeSeries/NPC.summary.marker.abun.pdf",width=4,height=10)
  ggsave(psPD,file="../../Result/TEST/timeSeries/NPC.EnrichPD.marker.abun.pdf",width=5,height=8)
  ggsave(psSD,file="../../Result/TEST/timeSeries/NPC.EnrichSD.marker.abun.pdf",width=4,height=8)
  ggsave(psPR,file="../../Result/TEST/timeSeries/NPC.EnrichPR.marker.abun.pdf",width=12,height=8)
}
```
